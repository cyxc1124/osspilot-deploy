...
镜像 tag 用 `IMAGE_TAG`（默认 `develop`）。应用镜像 `linux/amd64` + `linux/arm64`，`pull_policy: always`，每次 up 拉最新 `develop`。运营 API 把同一 tag 注入六个 `OSSPILOT_*_VERSION`，给关于页和 GitHub 比对用；Helm 里空的 `about.components` 回落到 `image.tag` / `global.imageTag`。
